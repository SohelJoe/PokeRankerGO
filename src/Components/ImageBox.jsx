...
const ImageBox = ({ id, form = null, name, megaClassName, className, w = 64, q = 75 }) => {
    ...
    return (<>
        ...
        <img className={!loaded ? 'hidden' : className} src={`https://db.pokemongohub.net/_next/image?url=%2Fimages%2Fofficial%2Ffull%2F${('000' + id).slice(-3)}${form == 'Mega Y' ? '_f3' : ['Hisuian', 'Galar'].includes(form) ? '_' + form.toLowerCase() : form ? '_f2' : ''}.webp&w=${w}&q=${q}`} alt={name} onLoad={() => setLoaded(true)} />
    </>)
}
